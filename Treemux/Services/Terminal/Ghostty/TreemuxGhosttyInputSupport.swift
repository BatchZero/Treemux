//
//  TreemuxGhosttyInputSupport.swift
//  Treemux
//
...
// MARK: - Text input routing

enum TreemuxGhosttyTextInputRouting {
    /// Returns true when the key event should bypass text input interpretation.
    ...
    /// Returns true when a raw key event should be marked as composing.
...
struct TreemuxGhosttyMarkedTextState: Equatable {
    var text: String
...
// MARK: - Equivalent key resolution

struct TreemuxGhosttyEquivalentKeyResolution: Equatable {
...
func resolveGhosttyEquivalentKey(
...
) -> TreemuxGhosttyEquivalentKeyResolution {
    switch charactersIgnoringModifiers {
    case "\r":
        guard modifierFlags.contains(.control) else {
            return TreemuxGhosttyEquivalentKeyResolution(equivalent: nil, nextLastPerformKeyEvent: lastPerformKeyEvent)
        }
        return TreemuxGhosttyEquivalentKeyResolution(equivalent: "\r", nextLastPerformKeyEvent: nil)
    ...
    case "/":
        ...
        return TreemuxGhosttyEquivalentKeyResolution(equivalent: "_", nextLastPerformKeyEvent: nil)
    ...
    default:
        ...
        guard modifierFlags.contains(.command) || modifierFlags.contains(.control) else {
            return TreemuxGhosttyEquivalentKeyResolution(equivalent: nil, nextLastPerformKeyEvent: nil)
        }

        if let lastPerformKeyEvent, lastPerformKeyEvent == eventTimestamp {
            return TreemuxGhosttyEquivalentKeyResolution(
                equivalent: characters ?? "",
                nextLastPerformKeyEvent: nil
            )
        }

        return TreemuxGhosttyEquivalentKeyResolution(equivalent: nil, nextLastPerformKeyEvent: eventTimestamp)
    }
}

// MARK: - NSEvent key event extensions
