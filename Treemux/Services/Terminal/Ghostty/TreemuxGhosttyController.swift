//
//  TreemuxGhosttyController.swift
//  Treemux
//
...
/// Actions that the terminal controller dispatches to the workspace layer.
enum TerminalWorkspaceAction {
    case createSplit(axis: SplitAxis, placement: PaneSplitPlacement)
...
/// Split placement relative to the focused pane.
enum PaneSplitPlacement {
    case before
    case after
}

/// Focus direction within the pane layout.
...
@MainActor
final class TreemuxGhosttyController: ManagedTerminalSessionSurfaceController {
    ...
    private let terminalView: TreemuxGhosttySurfaceView
    ...
    init(launchConfiguration: TerminalLaunchConfiguration) {
        ...
        self.terminalView = TreemuxGhosttySurfaceView()
    ...
    func beginSearch(initialText: String?) {
        if let initialText, !initialText.isEmpty {
            _ = terminalView.performBindingAction(treemuxGhosttySearchBindingAction(for: initialText))
            return
        }
    ...
    func updateSearch(_ text: String) {
        _ = terminalView.performBindingAction(treemuxGhosttySearchBindingAction(for: text))
    }

    func searchNext() {
        _ = terminalView.performBindingAction(treemuxGhosttySearchNavigationBindingAction(.next))
    }

    func searchPrevious() {
        _ = terminalView.performBindingAction(treemuxGhosttySearchNavigationBindingAction(.previous))
    }
    ...
    func startManagedSessionIfNeeded() {
        terminalView.ensureSurface(runtime: TreemuxGhosttyRuntime.shared, launchConfiguration: launchConfiguration)
        terminalView.syncSurfaceMetrics()
    }

    func restartManagedSession() {
        terminalView.recreateSurface(runtime: TreemuxGhosttyRuntime.shared, launchConfiguration: launchConfiguration)
        terminalView.syncSurfaceMetrics()
    }
    ...
    func handleGhosttyAction(_ action: ghostty_action_s, on surface: ghostty_surface_t) -> Bool {
        ...
        case GHOSTTY_ACTION_SCROLLBAR:
            terminalView.scrollbarState = TreemuxGhosttyScrollbarState(action.action.scrollbar)
            return true
        ...
        case GHOSTTY_ACTION_SECURE_INPUT:
            // TODO: Task 15 — secure input manager
            return true
    ...
    func handleSurfaceClose(processAlive: Bool) {
        guard treemuxGhosttyShouldReportProcessExitForSurfaceClose(processAlive: processAlive) else { return }
    ...
    func handleCommandFinished(_ action: ghostty_action_command_finished_s) {
        ...
        _ = treemuxGhosttyShouldReportProcessExitForCommandFinished(action)
    }
    ...
    func confirmClipboardRead(
        ...
        alert.accessoryView = TreemuxClipboardPreviewView(text: text)
    ...
    func confirmClipboardWrite(text: String, location: ghostty_clipboard_e) {
        confirmClipboardWrite(
            items: [TreemuxGhosttyClipboardPayload(mimeType: "text/plain;charset=utf-8", text: text)],
            location: location
        )
    }

    func confirmClipboardWrite(items: [TreemuxGhosttyClipboardPayload], location: ghostty_clipboard_e) {
        let alert = NSAlert()
        ...
        alert.accessoryView = TreemuxClipboardPreviewView(text: previewText)

        guard alert.runModal() == .alertFirstButtonReturn,
              let pasteboard = treemuxGhosttyPasteboard(for: location) else {
            return
        }
...
func treemuxGhosttyShouldReportProcessExitForSurfaceClose(processAlive: Bool) -> Bool {
    !processAlive
}

func treemuxGhosttyShouldReportProcessExitForCommandFinished(
...
private struct TreemuxGhosttyScrollbarState {
    let total: UInt64
    let offset: UInt64
    let length: UInt64
...
@MainActor
private final class TreemuxGhosttySurfaceView: NSView {
    ...
    var scrollbarState: TreemuxGhosttyScrollbarState? {
        didSet { controller?.notifyStatusChange() }
    }
    ...
    func ensureSurface(runtime: TreemuxGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        guard surface == nil else { return }
        createSurface(runtime: runtime, launchConfiguration: launchConfiguration)
    }

    func recreateSurface(runtime: TreemuxGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        destroySurface()
        createSurface(runtime: runtime, launchConfiguration: launchConfiguration)
    }
    ...
    func destroySurface() {
        ...
        if let surfaceUserdataToken {
            TreemuxGhosttyControllerRegistry.shared.unregister(surfaceUserdataToken)
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if textModifiers.isEmpty {
            ...
            sendRawKeyEvent(
                ...
                translationEvent: translationEvent,
                translationMods: translationMods,
                composing: TreemuxGhosttyTextInputRouting.shouldMarkRawKeyEventAsComposing(
    ...
    @IBAction func findNext(_ sender: Any?) {
        _ = performBindingAction(treemuxGhosttySearchNavigationBindingAction(.next))
    }

    @IBAction func findPrevious(_ sender: Any?) {
        _ = performBindingAction(treemuxGhosttySearchNavigationBindingAction(.previous))
    }
    ...
    @IBAction override func performTextFinderAction(_ sender: Any?) {
        guard let action = treemuxTextFinderAction(for: sender) else {
    ...
    func readSelection(from pboard: NSPasteboard) -> Bool {
        guard let string = pboard.treemuxGhosttyBestString else { return false }
        sendText(string)
        return true
    }
    ...
    private func createSurface(runtime: TreemuxGhosttyRuntime, launchConfiguration: TerminalLaunchConfiguration) {
        guard let controller else { return }

        let surfaceUserdataToken = TreemuxGhosttyControllerRegistry.shared.register(controller)
    ...
    private func shouldPreferRawKeyEvent(for event: NSEvent) -> Bool {
        TreemuxGhosttyTextInputRouting.shouldPreferRawKeyEvent(
            keyCode: event.keyCode,
            modifierFlags: event.modifierFlags
        )
    }
    ...
    private func deleteBackwardInMarkedText() {
        ...
        var state = TreemuxGhosttyMarkedTextState(text: markedText.string, selectedRange: markedSelectionRange)
    ...
    private func draggedText(from pasteboard: NSPasteboard) -> String? {
        ...
        return treemuxTerminalDropText(fileURLs: fileURLs, plainText: pasteboard.string(forType: .string))
    }
}

// MARK: - NSServicesMenuRequestor conformance

extension TreemuxGhosttySurfaceView: @preconcurrency NSServicesMenuRequestor {}

// MARK: - NSMenuItemValidation conformance

extension TreemuxGhosttySurfaceView: NSMenuItemValidation {}

// MARK: - NSTextInputClient conformance

extension TreemuxGhosttySurfaceView: @preconcurrency NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        return TreemuxGhosttyMarkedTextState.clamp(markedSelectionRange, textLength: markedText.length)
    }
...
private final class TreemuxClipboardPreviewView: NSScrollView {
    init(text: String) {
