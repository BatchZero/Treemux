...
extension SidebarItemIcon {
    ...
    static func randomRepository() -> SidebarItemIcon {
        randomRepository(avoiding: [])
    }

    static func randomRepository(avoiding existingIcons: [SidebarItemIcon]) -> SidebarItemIcon {
...
extension SidebarIconCatalog {
    struct RepositorySemanticProfile {
    ...
    struct RepositoryStylePreferences {
        let seed: UInt64
    ...
    static let repositoryCandidates: [SidebarItemIcon] = repositorySymbolNames.flatMap { symbolName in
    ...
    static func seededCandidateRank(for icon: SidebarItemIcon, seed: UInt64) -> UInt64 {
        mix64(seed ^ stableHash("\(icon.symbolName)|\(icon.palette.rawValue)|\(icon.fillStyle.rawValue)"))
    }

    private static func seededSymbolNames(seed: UInt64) -> [String] {
    ...
    private static func seededPalettes(seed: UInt64) -> [SidebarIconPalette] {
    ...
    private static func semanticProfile(for normalizedName: String) -> RepositorySemanticProfile? {
