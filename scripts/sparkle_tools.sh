...
# Sparkle helper functions for Treemux release pipeline.
...
sparkle_checkout_dir() {
  local root_dir="${1:-$(sparkle_repo_root)}"
  local project_path="${2:-$root_dir/Treemux.xcodeproj}"
  local scheme="${3:-Treemux}"
...
sparkle_tool_path() {
  ...
  local project_path="${3:-$root_dir/Treemux.xcodeproj}"
  local scheme="${4:-Treemux}"
...
sparkle_codesign_app() {
  ...
  /usr/bin/codesign \
    --force --sign "$signing_identity" --options runtime --timestamp \
    --preserve-metadata=entitlements,requirements,flags \
    "$app_path"
}
...
sparkle_generate_appcast() {
  ...
  local project_path="${9:-$root_dir/Treemux.xcodeproj}"
  local scheme="${10:-Treemux}"
