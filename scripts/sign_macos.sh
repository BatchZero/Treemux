...
Options:
  ...
  --version <version>    Override MARKETING_VERSION.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --identity)     require_value "$1" "${2:-}"; SIGNING_IDENTITY="$2"; shift 2 ;;
    --version)      require_value "$1" "${2:-}"; VERSION="$2"; shift 2 ;;
    --output-dir)   require_value "$1" "${2:-}"; OUTPUT_DIR="$2"; shift 2 ;;
    --app)          require_value "$1" "${2:-}"; APP_BUNDLE_PATH="$2"; shift 2 ;;
    --dmg)          require_value "$1" "${2:-}"; DMG_PATH="$2"; shift 2 ;;
    --release-archs) require_value "$1" "${2:-}"; RELEASE_ARCHS="$2"; shift 2 ;;
...
for cmd in codesign security xcodebuild xcrun /usr/bin/ditto /usr/bin/hdiutil; do
  require_cmd "$cmd"
done
...
if [[ "$NOTARIZE" == "1" ]]; then
  if [[ -z "$NOTARYTOOL_PROFILE" ]]; then
    echo "Notarization credentials missing. Set NOTARYTOOL_PROFILE." >&2
    exit 1
  fi
